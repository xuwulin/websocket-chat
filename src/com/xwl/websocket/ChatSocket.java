package com.xwl.websocket;

import com.alibaba.fastjson.JSON;
import com.xwl.util.MessageUtil;
...
// websocket请求的url：/websocket
@ServerEndpoint(value = "/websocket", configurator = GetHttpSessionConfigurator.class)
public class ChatSocket {

    // websocket会话的session
    private Session session;
    // 登录用户的session（servlet中的session）
    ...
    /**
     * 当开启一个新的会话时调用，该方法是客户端和服务端握手成功后调用的方法
     * @param session
     * @param config
     */
    @OnOpen
    ...
    /**
     * 在线人数加一
     * onlineCount++ 不是一个原子操作，存在并发问题，需要使用 synchronized 关键字解决
     */
